...
struct Telefon* copiazaPrimeleNElemente(struct Telefon* vector, int nrElemente, int nrElementeCopiate) {
	...
	// alocare spatiu vectorNou
	vectorNou = (struct Telefon*)malloc(sizeof(struct Telefon) * nrElementeCopiate);
	for (int i = 0; i < nrElementeCopiate; i++) {
		// aici ma folosesc de shallow copy ptr ca am nevoie ptr celelalte atribute, in afara de char* producator
		vectorNou[i] = vector[i];
		//deep copy nu shallow copy
...
int main() {
	...
	int nrTelefoane = 3;
	// alocare spatiu pentru vectorul meu telefoane
